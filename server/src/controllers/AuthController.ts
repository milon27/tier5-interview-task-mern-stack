...
const AuthController = {
    signUp: async (req: Request, res: Response) => {
        try {
            ...
            //create the new user.
            const user = await req.prisma.admins.create({
                data: {
                    name: name,
                    email,
                    password: hashpass
            ...
            res.status(200).json(MyResponse<Admins>("admin created successfully", user))
        } catch (e) {
    ...
    login: async (req: Request, res: Response) => {
        try {
            ...
            const u = await req.prisma.admins.findUnique({
                where: {
                    email: email
                }
            })
            ...
            //send token in http cookie with no expire
            res.cookie(Define.TOKEN, token, Helper.getSessionCookieOption(req.agent))
            res.status(200).json(MyResponse<Admins>("user loggedin successfully", user))
        } catch (e) {
    ...
    getLoggedInUser: async (req: Request, res: Response) => {
        try {
            ...
            const user = u!
            res.status(200).json(MyResponse<Admins>("got admin successfully", user))
        } catch (e) {
